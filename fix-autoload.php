...
$static_loader = preg_replace('/\'([A-Za-z0-9]*?)\' => __DIR__ \. (.*?' . $match . '.*?),/', '\'' . $prefix . '$1\' => __DIR__ . $2,', $static_loader);
...
$files_loader = preg_replace('/\'(.*?)\' => (.*?' . $match . '.*?),/', '\'' . $prefix . '$1\' => $2,', $files_loader);
